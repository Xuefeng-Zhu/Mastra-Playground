...
const provider = z.enum(['google', 'openrouter']).optional();
...
export const EXAMPLE_INPUT_SCHEMAS = {
  ...
  research: z.object({ topic: requiredText('topic'), provider, model }),
  'code-review': z.object({ path: text('path', 512).default(''), provider, model }),
  'parallel-research': z.object({ topic: requiredText('topic'), provider, model }),
  ...
  'streaming-chat': z.object({ prompt: requiredText('prompt'), provider, model }),
  ...
  'multi-agent-handoff': z.object({ message: requiredText('message'), provider, model }),
